...
class LicenseRemoteDatasourceImpl extends LicenseRemoteDatasource {
  final CollectionReference<LicenseModel> licenseFirestoreRef =
      FirebaseFirestore.instance
          .collection('Licenses')
          .withConverter<LicenseModel>(
              fromFirestore: (snapshot, options) =>
                  LicenseModel.fromJson(snapshot.data()!, snapshot.id),
              toFirestore: (license, options) => license.toJson());
  ...
  Future<String> addLicense(
      ...
      String status,
      String user_id,
      DateTime register_date) async {
    //Subiendo la imagen de justificación a Firebase
    String justification = '';
    if (justificationFile != null) {
      justification = await storage.uploadFile(justificationFile, 'Licenses');
    }
  ...
  Future<List<LicenseModel>> getLicenses() async {
    ...
    print(licensesDocs.docs.length);
  ...
  Future<List<LicenseModel>> getLicensesByStudentID(String user_id) async {
    final licensesDocs =
        await licenseFirestoreRef.where('user.id', isEqualTo: user_id).get();
    final licenses = licensesDocs.docs.map((e) {
      print('License ${e.data()}');
      return e.data();
    });
    final listLicenses = licenses.toList();
