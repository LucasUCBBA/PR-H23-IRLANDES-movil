...
abstract class NotificationRemoteDataSource {
  ...
  Future<List<NotificationModel>> getNotificationByUserToken(String token);
}

class NotificationRemoteDataSourceImpl extends NotificationRemoteDataSource {
  final CollectionReference<NotificationModel> notificationFirestoreRef =
  ...
  Future<NotificationModel> getNotificationByToken(String token) async {
    final notificationDoc = await notificationFirestoreRef
  ...
  Future<List<NotificationModel>> getNotificationByUserToken(
      String token) async {
    List<NotificationModel> notifications = [];
    final notificationDoc = await notificationFirestoreRef
        .where('deviceToken', isEqualTo: token)
        .get();
    for (var not in notificationDoc.docs) {
      notifications.add(not.data());
    }
